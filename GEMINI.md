...
- **Automated Testing Suite**: Vitest + JSDOM (`npm run test`), 37/37 проходящих автотестов для утилит и SaaS функционала.
...
- **SaaS Features & DevTools Suite (`saasFeatures.ts` & `devTools.ts`)**:
  ...
  - **SaaS Snippet Vault & Custom Templates (`Ctrl+Shift+V`)**: Встроенный каталог шаблонов (Docker, Nginx, Postgres, OpenAPI, Vite React) + форма создания и локального сохранения собственных сниппетов.
  - **Developer Tools**: Генератор TypeScript интерфейсов из JSON, SQL Форматтер & Минификатор, конвертер cURL в `fetch()`, Unicode Base64, JWT Декодер, CSV -> Markdown Таблицы, URL/HTML Encode & Decode.
...
3. **Unicode Base64 & Advanced DevTools**: Нативный Unicode Base64 кодер (`TextEncoder`/`TextDecoder`), генератор TS-типов из JSON, SQL форматирование, cURL в fetch, JWT парсер.
...
7. **Smart Context Floating Bar**: Авто-детекция выделенного текста (Unix Timestamp -> дата, JSON -> TS Interface, SQL -> Format, Base64 -> расшифровка, `Ctrl+K` -> AI).
...
└── src/
    ...
    │   ├── QuickContextBar.tsx       # Всплывающая плашка на выделение (Timestamp/Base64/JSON->TS/SQL)
    ...
    └── utils/
        ...
        ├── devTools.ts               # Safe Unicode Base64, JWT, JSON, SQL, TS Generator, cURL, Log Cleaner
...
# 4. Запуск suite автотестов (Vitest, 37/37 tests)
npm run test
